...
namespace net_http
{
    ...
    {
        //LOG_INFO << "HttpParser::onParser event: " << event;
        switch(event)
        {
    ...
    int HttpParser::execute(const char* buffer, size_t count)
    {
        ...
        //LOG_INFO << "HttpParser::execute: m_parser.upgrade " << m_parser.upgrade << " count: " << count; 
        if(m_parser.upgrade)
        {
        ...
        else if(n != count)
        {
            ...
            LOG_ERROR << "parser error " << error.message.c_str();
