...
{
  LOG_INFO << "ProtobufCodec::onMessage - recv size: " << buf->readableBytes();
  while (buf->readableBytes() >= kMinMessageLen + kHeaderLen)
  {
    ...
    if (len > kMaxMessageLen || len < kMinMessageLen)
    {
      ...
      LOG_INFO << "ProtobufCodec::onMessage error";
      break;
    }
    else if (buf->readableBytes() >= implicit_cast<size_t>(len + kHeaderLen))
    {
      LOG_INFO << "ProtobufCodec::onMessage2";
    ...
    {
      LOG_INFO << "ProtobufCodec::onMessage3";
      break;
    }
  }
}
