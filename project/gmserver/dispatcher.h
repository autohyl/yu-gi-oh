...
public:
    ...
    explicit ProtobufDispatcher(const ProtobufMessageCallback& defaultCb, GmServer* gmserve)
                          ...
                          muduo::Timestamp receiveTime) const
    {
		...
		BaseHandlePtr handle;
		handle.reset();
		//生产出处理对应消息的handle
		bool ret = HandleFactory::createHandle(message, handle, gmserve_);
		if (ret) {
			handle->execute();
		}
		else {
